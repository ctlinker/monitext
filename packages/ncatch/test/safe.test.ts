...
import { createSafeExecutor } from '../src/lib/safe';
...
describe('createSafeExecutor', () => {
	describe('sync functions', () => {
		...
		it('returns [error, null] on throw', () => {
			const safe = createSafeExecutor(() => {
				throw new Error('boom');
			}); // ts leak, the fn never reach it's end, type widened to unknown
			const [err, data] = safe();
	...
	describe('async functions', () => {
		...
		it('returns [error, null] on rejection', async () => {
			const safe = createSafeExecutor(async () => {
